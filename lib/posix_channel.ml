...
module CBuf = struct
  (** A circular buffer constructed from the bytes type *)
  type t = {
        ...
        let close fd =
          ...
          (fun () -> 
            ...
            if List.mem s_unix readable then begin
              ...
              if token = (Bytes.to_string token') then begin
  ...
  | Unix_sendmsg(_, path, token) ->
    let s = Unix.socket Unix.PF_UNIX Unix.SOCK_STREAM 0 in
      ...
      (fun () ->
        ...
        let (_: int) = Unix.send s (Bytes.of_string token) 0 (String.length token) [] in
